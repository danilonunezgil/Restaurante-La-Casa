...
from pages.views import embed_page_key, PageNotFoundError, PageAuthorizationError
from pages.models import PageGroup
...
class EmbeddedPageNode( template.Node ):
    ...
    def render(self, context):
        if not isinstance( context, template.RequestContext ):
            return node_error( _("Context must be of type RequestContext.") )
        try:
            key = self.page_key
            ...
            return node_error( _("The embedded page could not be found.") )
        except PageAuthorizationError:
            return node_error( _("User not authorized to view page.") )
        except template.VariableDoesNotExist:
            return node_error( _("Couldn't resolve request variable: %s")  )
...
def embed_page_no_unpublished( parser, token ):
    ...
    if len(tokens) != 2:
        raise template.TemplateSyntaxError(_( "embed_page tag requires one argument" ))
