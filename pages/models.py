...
#from djangoplicity.translation.models import TranslationModel, \get_path_for_language

if not hasattr( settings, 'PAGE_TEMPLATE_CHOICES'):
    settings.PAGE_TEMPLATE_CHOICES = ()

CACHE_KEY = {
    'embedded_pages': 'pages_embedded',
    'pages': 'pages_',
    'urlindex': 'pages_urlindex',
    'embedded_page_keys': 'pages_keys_index',
}
...
class URL( models.Model ):
    ...
    url = models.CharField(
        ...
        db_index=True,
        unique=True,
        help_text=_("Example: '/about/contact/'. Make sure to have leading and trailing slashes. Good and descriptive URLs are important for good user experience and search engine ranking.")
    )
    page = models.ForeignKey('Page', verbose_name=_('Page'), null=True, on_delete=models.CASCADE )
...
class PageGroup( models.Model ):
    ...
    '''
    name = models.CharField(verbose_name= _('Name'),max_length=50, help_text=_('Name of the group'))
    description = models.TextField(verbose_name=_('Description'),blank=True, help_text=_('Description of group'))
    groups = models.ManyToManyField(Group, verbose_name=_('Group'), help_text=_('Groups which have to access to this page group'), blank=True)
...
class Page( models.Model ):
    ...
    # Page content
    #
    id = models.SlugField( primary_key=True, help_text=_('ID of page') )

    embedded = models.BooleanField(
        default=False,
        help_text=_('If checked, the page cannot be viewed through the URL. Used for pages that are embedded into complex pages like the frontpage.') 
    )
    ...
    title = models.CharField(
        ...
        max_length=200,
        help_text=_("Title is shown in browser window. Use a good informative title, since search engines normally display the title on their result pages.") 
    )
    ...
    script = models.TextField(blank=True, verbose_name=_('script'), help_text='Javascript to be included in page footer')
    ...
    description = models.TextField(
        ...
        blank=True,
        help_text=_('The metadata description is normally shown in search engine results, making the description an effective way of capturing users attention. Description should be a clear description of the content and less the 200 characters long. Also used when sharing page on social media') 
    )
    ...
    keywords = models.TextField(
        ...
        blank=True,
        help_text=_('Comma-separated list of keywords for this page. Mainly used internally as search engines rarely use keywords to rank pages.')
    )
    ...
    opengraph_image = models.CharField(
        verbose_name= _('OpenGraph Image'),
        blank=True, max_length=250,
        help_text=_("Example: 'https://www.eso.org/public/archives/imagecomparisons/newsfeature/potw1413a.jpg'. If given: full path to an image that will be used when sharing the page on social media. Must be larger than 200x200px and smaller than 5MB.")
    )

    #
    # Publishing information
    #
    #TOOD: Test if this should be blank, null or blank and null
    ...
    published = models.BooleanField(
        ...
        default=True,
        help_text=_('If this is unchecked, the page will not be viewable online. Timed publishing of published pages, can be controlled by the start/end publishing fields.') 
    )
    ...
    start_publishing = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_('Leave blank to publish immediately. The field only has effect for published pages (see above).') 
    )
    # Determines when the page should go online. The field only has effect if "published" field is true.

    end_publishing = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_('Leave blank for open-ended publishing. The field only has effect for published pages (see above).') 
    )
    ...
    # Page style
    #
    section = models.ForeignKey( Section, default=1, help_text=_('Determines e.g. which templates to use for rendering the template.'), on_delete=models.CASCADE )
    # Section to use for this page. A section determines default values for a page, and the visual layout (e.g. templaes).

    template_name = TemplateField(max_length=100, blank=True, help_text=_('Override the template specified by the section.'))
    ...
    redirect_url = models.CharField(_(u'Redirect URL'), blank=True, max_length=200, db_index=True,
            help_text=_("Example: 'https://www.eso.org/public/'. Used if the page has been permanently moved. If given, access to the page will trigger a HTTP 301 redirection to the given URL."))
...
class EmbeddedPageKey( models.Model ):
    ...
    description = models.TextField( 
        blank=True, 
        help_text=_('Description of where the page is being embedded and if there is any special considerations to take - e.g. page length etc.') 
    )
    ...
    page = models.ForeignKey( 
        ...
        null=True, 
        blank=True, 
        limit_choices_to={ 'embedded__exact': True }, 
        help_text=_('Select page that you want to use for the specific key. Note, only pages marked as embedded pages can be selected.'), 
