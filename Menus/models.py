...
from mptt.models import MPTTModel # importar en requeriments y settings
from operator import itemgetter

CACHE_KEY = 'Menus.menu_'
...
class MenuItem( MPTTModel ):
    ...
    menu = models.ForeignKey( Menu, blank=True, null=True, on_delete=models.CASCADE,help_text=_( u'This field only have effect for the root menu item' ) )

    parent = models.ForeignKey( 'self', null=True, blank=True, related_name='children', on_delete=models.CASCADE, help_text=_( u'Moving a node to a new parent, will place it as the last node. When moving a menu item, all of it sub-items will be moved as well.') )
...
def menu_cache_dependencies( sender, instance, signal, *args, **kwargs ):
    ...
    '''
    from pages.models import Page, CACHE_KEY as PAGE_CACHE_KEY
